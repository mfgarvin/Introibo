...
- **Feedback Worker** (`worker/`) is **deployed** to Cloudflare at `https://introibo-feedback.mfgarvin.workers.dev` (D1-backed, account mfjgarvin@gmail.com). `lib/config/feedback_endpoint.dart` defaults to `…/feedback`, so submissions are live; override per-build with `--dart-define=FEEDBACK_ENDPOINT=…`. Redeploy/inspect via `worker/README.md` (`wrangler deploy`, `wrangler d1 execute …`).
...
- **Build number** is git-derived (`git rev-list --count HEAD`) on Android; iOS native side is not yet wired.
