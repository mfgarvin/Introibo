...
- **Feedback Worker** (`worker/`) is **deployed** to Cloudflare on two hostnames serving the same Worker (D1-backed, account mfjgarvin@gmail.com): `https://api.parishfinder.app` (preferred — on our own zone, so WAF/rate-limiting rules can reach it) and the legacy `https://introibo-feedback.mfgarvin.workers.dev`, kept enabled because shipped beta APKs point at it. The Worker/D1 keep the old `introibo-feedback` name on purpose. `lib/config/feedback_endpoint.dart` defaults to `https://api.parishfinder.app/feedback`, so submissions are live; override per-build with `--dart-define=FEEDBACK_ENDPOINT=…`. Redeploy/inspect via `worker/README.md` (`wrangler deploy`, `wrangler d1 execute …`).
